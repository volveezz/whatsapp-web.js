...
exports.LoadUtils = () => {
    ...
    window.WWebJS.processMediaData = async (
        ...
        console.log(
            `${logPrefix} [WWebJS:processMediaData] Uploaded Media, took ${
                end - start
            } ms`,
        );
    ...
    window.WWebJS.getMessageModel = (message) => {
        ...
        if (message.richPreviewType && message.isLink) {
            msg.previewType = message.richPreviewType;
            msg.previewSrc = message.body;
            msg.isLink = true;

            // Extract additional preview data
            if (message.preview && message.preview.thumbnail) {
                msg.previewThumbnailUrl = message.preview.thumbnail;
            }
            if (message.matchedText) {
    ...
    /**
     * Downloads and returns a link preview thumbnail as a MessageMedia object
     * @param {string} thumbnailUrl - URL of the thumbnail to download
     * @returns {Promise<{mimetype: string, data: string, filename: string}|null>} - MessageMedia properties or null if download failed
     */
